...
export interface SampleTimelineItem {
  time: string;
  label: string;
}
...
export interface InvitationPhoneMockupViewProps {
  theme: ThemeKey;
...
// The presentational core of the phone mockup — no hooks, no server-only calls, so it's a legal
// import from both a Server Component (InvitationPhoneMockup.tsx) and a Client Component
// (InstantPreview.tsx), the same way THEME_STYLES already is. Callers resolve `labels` themselves
// (via getTranslations server-side, or plain useTranslations client-side).
export function InvitationPhoneMockupView({
  theme: themeKey,
  eventTypeLabel,
  name,
  dateLabel,
